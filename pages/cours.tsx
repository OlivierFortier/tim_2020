...
export default function Cours({ listeCours }: { listeCours: Cours[] }) {
  ...
  function filtrerCours(evenement: ChangeEvent<HTMLSelectElement>) {
    ...
    const coursFiltres = listeTousLesCours.map((session) =>
      ...
      [...session].filter((cours) => {
        const sujetPrincipal = cours.sujetPrincipal.some((typeCours: string) =>
          evenement.target.value === ""
            ? typeCours
            : typeCours === evenement.target.value
        );

        const sujetsSecondaires = cours.sujetsSecondaires?.some(
          (typeCours: string) =>
            evenement.target.value === ""
              ? typeCours
              : typeCours === evenement.target.value
              ...
              >
                {coursAffiche ? (
                  <DetailsCours
                    ...
                    afficherCours={() => setCoursAffiche("")}
                  />
                ) : (
                  session.map(
                    (
                      coursFiltre: SetStateAction<string>,
                      indexCours: number
                    ) => (
                      <NomCours
...
type Cours = {
  ...
  session: number;
  auChoix: boolean;
  choixEntre: { titre: string }[];
  sujetPrincipal: string;
  sujetsSecondaires: string[];
  logicielsEtTechnologies: string[];
};
