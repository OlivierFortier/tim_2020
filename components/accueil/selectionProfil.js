...
export default function SelectionProfil({ changerEtape }) {
  ...
  return (
    <>
      <div className={styles.conteneurProfils}>
        <h1>Sélectionnez votre profil</h1>
        <div className={styles.grilleProfils}>
          <span onClick={() => choisirTheme(listeThemes.art)} className={styles.art}>
            <h2>Artiste Digital</h2>
          </span>
          <span onClick={() => choisirTheme(listeThemes.code)} className={styles.code}>
            <h2>
              Pirate <br /> Informatique
            </h2>
          </span>
          <span onClick={() => choisirTheme(listeThemes.parent)} className={styles.parent}>
            <h2>Parent</h2>
          </span>
          
        </div>
        <h2 className={styles.rapide} onClick={() => choisirTheme(listeThemes.parent)}>Accéder au site directement</h2>
      </div>
    </>
  );
}
